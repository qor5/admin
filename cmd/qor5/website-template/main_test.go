...
func TestServerCreationWithRealHandlers(t *testing.T) {
	// Test with actual handlers used in main()
	t.Run("CMS server creation", func(t *testing.T) {
		// Skip if we can't connect to DB (integration test environment)
		defer func() {
			if r := recover(); r != nil {
		...
		// This would test the actual CMS server creation
		// In a real scenario, you might want to mock the admin.Router and admin.ConnectDB calls
		handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.WriteHeader(http.StatusOK)
		})

		server := serverWithTimeouts(":9001", handler)

		if server.Addr != ":9001" {
			t.Errorf("Expected addr :9001, got %s", server.Addr)
		}
	...
	t.Run("Publish server creation", func(t *testing.T) {
		// Test file server creation
		handler := http.FileServer(http.Dir("."))
		server := serverWithTimeouts(":9002", handler)

		if server.Addr != ":9002" {
			t.Errorf("Expected addr :9002, got %s", server.Addr)
		}
