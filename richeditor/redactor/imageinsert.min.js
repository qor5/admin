Redactor.add('plugin', 'imageinsert', {
    ...
    start: function(){
        ...
        var $button = this.toolbar.addButton('image', buttonData);

        
       this.hiddenContainer = this.app.container.getElement().nextElement().find(".go-plaid-portal").first()
       
       var that = this;
    //    checking dom change and update input data
    var observe = new MutationObserver(function(mutations, observer){
        that.selectImageInput = that.hiddenContainer.find("input");
        that.inputImageValue = that.selectImageInput?that.selectImageInput.val():"null";
        that.insertQorImage();
    });
    ...
    insertQorImage:function(){
        if(this.inputImageValue!="null"){
            ...
            inputImageJSON.forEach(image => {
                var data = {id:image.ID,title:image.FileName,url:image.Url,alt:image.Description};
