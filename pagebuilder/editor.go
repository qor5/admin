			...
			VCard(
				h.Tag("vx-draggable").
					...
					h.Template(
						VListItem(
							web.Slot(
								...
								h.If(!isReadonly,
									...
									VBtn("").Icon(
										VIcon("{{element.visibility_icon}}").Size(SizeSmall),
									).Attr("@click",
										web.Plaid().
						...
						).Class("pl-0").Attr("@click", fmt.Sprintf(`document.querySelector("iframe").contentWindow.postMessage(%s+"_"+%s,"*");`, web.Var("element.model_name"), web.Var("element.model_id"))),
