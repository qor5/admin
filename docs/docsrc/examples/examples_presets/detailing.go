...
func PresetsDetailDisableSave(b *presets.Builder, db *gorm.DB) (
	...
	sec.EditingField("Btn").ComponentFunc(func(obj interface{}, field *presets.FieldContext, ctx *web.EventContext) h.HTMLComponent {
		return h.Div(
			vx.VXBtn("Disabled").Attr("@click", fmt.Sprintf("dash.%s_disabledSectionSave=true", "DisabledSectionCompany")).Color(ColorSecondary),
			vx.VXBtn("Savable").Attr("@click", fmt.Sprintf("dash.%s_disabledSectionSave=false", "DisabledSectionCompany")).Color(ColorPrimary),
		).Class("d-flex d-flex-inline ga-4")
	})
	dpc.Section(sec)
	return
